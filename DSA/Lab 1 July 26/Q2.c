...
for (i=0; i<4; i++)
{
 printf("\nTotal votes of Candidate %c is %d and Percentage is %.2f ", candidateName[j++], sum[i], ((float)sum[i]/cumalativeSum)*100);
}
...
for (i=0; i<4; i++)
{
 per = ((float)sum[i]/cumalativeSum)*100;
...
printf("\nRunning Code with candidate C receiving only 108 votes in Precinct 4\n\n");
